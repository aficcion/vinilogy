...
- **Conexión**: añade el plugin Postgres en el proyecto de Railway y pon
  `VINYLBE_DB_DSN` = la connection string interna (p.ej. referenciando
...
- Vigila `max_connections` del plan de Railway vs `VINYLBE_DB_POOL_MAX` (def. 24).
...
|---|---|---|
| `VINYLBE_DB_DSN` | DSN de la Postgres con `vinology_core` (prod) | Bloqueante #1. Con `?sslmode=require` si aplica |
| `VINYLBE_BASE_URL` | `https://<tu-dominio>` | Debe casar EXACTO con los redirect URIs registrados |
| `VINYLBE_SECURE_COOKIES` | `1` | Cookies con flag Secure (estás sobre HTTPS) |
...
| `VINYLBE_DB_POOL_MAX` | `24` (o según `max_connections`) | Opcional |
| `VINYLBE_LOG_LEVEL` | `INFO` | Opcional |
| `VINYLBE_DEV_LOGIN` | **NO DEFINIR** | `=1` sería bypass total de auth |
...
- [ ] `VINYLBE_DB_DSN` de prod resuelto (bloqueante #1) y probado.
...
- [ ] `VINYLBE_SECURE_COOKIES=1` y `VINYLBE_DEV_LOGIN` sin definir.
...
Estado: el repo local de v2 **no tiene remote**; el v1 vive en `aficcion/Vinylbe`
...
cd /ruta/al/clon/de/v1     # o un clon fresco de aficcion/Vinylbe
...
git remote add origin git@github.com:aficcion/Vinylbe.git
